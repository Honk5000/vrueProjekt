...
namespace CSharpSynth.Synthesis
{
    public class StreamSynthesizer
    {
		...
		private Dictionary<int, int> _channelProgramOverrides;
		/// <summary>
		/// A dictionary indicating fixed channel->program associations, i.e. overriding the actual program values in the MIDI.
		/// </summary>
		public Dictionary<int, int> channelProgramOverrides {
			get {
				if (_channelProgramOverrides == null) {
					_channelProgramOverrides = new Dictionary<int, int>();
				}
				return _channelProgramOverrides;
			}
			set {
				_channelProgramOverrides = value;
			}
		}
        ...
        public void NoteOn(int channel, int note, int velocity, int program)
        {
			...
			if (channelProgramOverrides != null && channelProgramOverrides.ContainsKey(channel)) {
				// We have fixed channel -> program associations specified for this note.
				// Override the actual specified program.

				freeVoice.setInstrument(bank.getInstrument(channelProgramOverrides[channel], (channel==9)));
